...
# Part I: loading the datasets and running and saving the model results. 
...
# If you take a look at the likelihood section of the model (lines 652-664), we use the categorical
# distribution: dcat(). 

# This distribution cannot work with zeros, so we must add 1 to our capture-recapture histories:
# See lines from 56 to 61 
...
# Ensure we have the nodes needed to simulate new datasets (in order words, the parts of the model
# that are needed for simulation)
dataNodes   <- survival$getNodeNames(dataOnly = TRUE) # y <- observations
parentNodes <- survival$getParents(dataNodes, stochOnly = T)  # `getParents` is new in nimble 0.11.0
simNodes    <- survival$getDependencies(parentNodes, self = FALSE)  

survivalConf <- configureMCMC(survival, monitors = parentNodes)  
...
survivalConf$printSamplers(c("beta", "eps_phi", "eps_psi"))
...
# Originally, the first part of the script ended here. After running the model, the samples had to be reloaded in a new job in the 
# supercomputer.

# Part II: we load the samples of the model and start simulating new datasets.

## Load data
samples <- readRDS("./samples.rds")

# Now, we create an empty matrix, we the number of rows being equal to the number of iterations less the number of burnin' samples.
# The number of columns = number of data Nodes. 
ppSamples <- matrix(0, nrow = n.iter-n.burnin, ncol = length(survival$expandNodeNames(dataNodes, returnScalarComponents = TRUE)))

# We have to make sure that parameters are in the same order. 
colnames(ppSamples) <- survival$expandNodeNames(dataNodes)
vars <- CsurvivalMCMC$mvSamples$getVarNames()
# colnames(samples) 
# vars

# Time to simulate! We will only use one chain, after we have checked that there are no
# issues with convergence etc. 

system.time({
  for(i in seq_len(n.iter-n.burnin)) {
    values(Csurvival, vars) <- samples[i, ]  # assign 'flattened' values
    Csurvival$simulate(simNodes, includeData = TRUE)
    ppSamples[i, ] <- values(Csurvival, dataNodes)
  }
})

# This part of the script also requires high amounts of memory. After its completion, the result is stored. 

saveRDS(ppSamples, "./ppSamples.rds")


# Part III: time to load the last output. Note. In this new session it is important to load the capture-recapture histories, the 
# vector containing sex information and the information of the first capture. 


##--Load data--------------------------------------------------------------------
#In my computer this doesnt work because the data is coma delimited but it might be the country setting. Maybe double check. 
starlings <- read.csv("./data/event_hist_new_cert1.csv", sep = ",", head = T) # dataset with the event histories
#sex <- starlings$sex # vector for sex; 1 = males; 2 = females
sex_dummy <- readRDS("./data/sex_new_vector.rds")
starlings <- starlings[,-c(1:4)] # I get rid of useless columns
...
# Load the output file of the Part II. 
ppSamples <- readRDS("./ppSamples.rds")


# Manipulating the output of the simulations ------ 
# 1.  Transposing the matrix to get the y[,] in the rows.
ppSamplesT <- t(ppSamples) # transpose matrix

# 2. Converting it to a dataframe to easily handle it. 
ppSamplesT_df <- as.data.frame(ppSamplesT) 

# 3. Some code to reformat outputs. 
# Add row numbers
ppSamplesT_df$row_num <- seq.int(nrow(ppSamplesT_df))   

### Convert the rownames into a variable itself. 
ppSamplesT_df$y_index <- rownames(ppSamplesT_df)  

### Putting those columns at the beginning of the dataframe. 
ppSamplesT_df <- ppSamplesT_df %>%   select(y_index, row_num, everything()) 

### Converting the y[1,2] into numbers to use it as a coordinate.
ppSamplesT_df$numbers  <- regmatches(ppSamplesT_df$y_index, gregexpr("[[:digit:]]+", ppSamplesT_df$y_index))            # Apply gsub

### Again: beginning of the dataframe. 
## row number the first in the dataframe.
ppSamplesT_df <- ppSamplesT_df %>%   select(y_index, row_num,numbers, everything()) 
### Splitting numbers into two columns
ppSamplesT_df <- ppSamplesT_df %>% unnest_wider(numbers)

### Eliminating the named numbers. I only want the numbers
ppSamplesT_df$...1 <- as.numeric(ppSamplesT_df$...1)
ppSamplesT_df$...2 <- as.numeric(ppSamplesT_df$...2)

## Renaming those columns
colnames(ppSamplesT_df)[which(names(ppSamplesT_df) == "...1")] <- "ind" # individuals
colnames(ppSamplesT_df)[which(names(ppSamplesT_df) == "...2")] <- "occ" # occasions

### Checking everything is fine
n.ind <- max(ppSamplesT_df$ind)
n.occ <- max(ppSamplesT_df$occ)

# print(n.ind) # ok 
# print(n.occ) # ok 


# With the information collected in our ppSamplesT_df, we will proceed to create the capture-recapture
# histories. 
# For multiple iterations. 

#get number of samples
nsamples <- dim(ppSamplesT_df)[2]-4 
# We create an empty matrix with the correct number of dimensions: nrows x ncols x nsamples
# number of rows = number of individuals
# number of columns = number of occasions
# nsamples = number of iteration
CH_ALL<-array(0, dim = c(n.ind, n.occ, nsamples))

# Filling the matrix with the first capture occasion of each individual. The same for each
# iteration.

for(i in 1:dim(CH_ALL)[1]){
  col<-first[i]  # remember to have this vector on your environment! 
  CH_ALL[i,col,]<-1 
}

# Searching in the ppSamplesT_df the coordinate for each occassion and for each individual and I give it the value of that 
# iteration.
for (iter in 1:dim(CH_ALL)[3]){
  for(i in 1:dim(ppSamplesT_df)[1]){
    iteracion <- iter + 4 # because I have four columns before the first column in iteration values.
    row <- as.numeric(ppSamplesT_df[i,3])
    col <- as.numeric(ppSamplesT_df[i,4])
    CH_ALL[row, col, iter] <- as.numeric(ppSamplesT_df[i, iteracion])
  } 
}

COUNT <- CH_ALL  

saveRDS(CH_ALL, "historiales_recaptura_original.rds")  #in case you are not continuing the session. If you save it, remember to 
# reload it in a new session to continue with the rest of the script.

# Now, we have to correctly append the sex of each individual and separate them to make specific 
# histograms for males and for females.

sex_vector <- sex # I duplicate it just in case

CH_with_sex <- abind(COUNT, array(sex_vector, replace(dim(COUNT), 2, 1)), along = 2) # we append it 

# We separate males from females.

d <- dim(CH_with_sex)
f <- function(i) CH_with_sex[i, , , drop = FALSE]
l <- c(tapply(seq_len(d[1L]), CH_with_sex[, d[2L], 1L], f, simplify = FALSE))

males<-   l[[1]]  # subset for males
females<- l[[2]]  # subset for females 

# print(dim(males)) # ok !
# print(dim(females)) # ok! 

# Plot posterior predictive checks histograms. 

# Males
y_males <- y[which(sex==1),] # Subset males (sex=1). 
# dim(y_males) # ok


# Counting breeders 
# Expected male breeders from our model
# Remember, in the model we used = y + 1; event 2 becomes a 3 (lines 51-61)
male_breeders <- colSums(males[,,]==3, dims = 1) # We sum across iterations
male_breedersT <- t(male_breeders) # transpose
male_breedersT_df <- as.data.frame(male_breedersT) # format as dataframe.
# Observed male breeders in our original dataset
male_breeders_originales <- as.numeric(colSums(y_males[,]==2,dims=1)) 

# Histogram
colnames(male_breedersT_df) <- (1:11)
jpeg("male_breeders.jpg", width = 12, height = 8, units = "in", res = 300)
male_breeders_hist <- recordPlot(
  par(mfrow=c(2,6)),
  for(i in 1:11){
    hist(male_breedersT_df[,i])                        # expected
    abline(v=male_breeders_originales[i], col = "red") # observed
  }
)

dev.off()

# Counting Floaters.  Event number = 3
# Expected male floaters from our model
# Remember, in the model we used = y + 1; event 3 becomes a 4 (lines 51-61)
male_floaters<- colSums(males[,,]==4, dims = 1)
male_floatersT <- t(male_floaters)
male_floatersT_df <- as.data.frame(male_floatersT) 
# Observed male floaters in our original dataset
male_floaters_originales <- as.numeric(colSums(y_males[,]==3,dims=1))

## Histogram
colnames(male_floatersT_df) <- (1:11)
jpeg("male_floaters.jpg", width = 12, height = 8, units = "in", res = 300)
male_floaters_hist <- recordPlot(
  par(mfrow=c(2,6)),
  for(i in 1:11){
    hist(male_floatersT_df[,i])                        # expected
    abline(v=male_floaters_originales[i], col = "red") # observed
  }
)
...
# Counting Unknowns.  Event number = 4

# Expected male unknown breeding status from our model
# Remember, in the model we used = y + 1; event 5 becomes a 5 (lines 51-61)
male_unknown <- colSums(males[,,]==5, dims = 1) 
male_unknownT <- t(male_unknown)
male_unknownT_df <- as.data.frame(male_unknownT) 

# Observed male unknown breeding status in our original dataset
male_unknown_originales <- as.numeric(colSums(y_males[,]==4,dims=1))

## Histogram
colnames(male_unknownT_df) <- (1:11)
jpeg("male_unknown.jpg", width = 12, height = 8, units = "in", res = 300)
male_unknown_hist <- recordPlot(
  par(mfrow=c(2,6)),
  for(i in 1:11){
    hist(male_unknownT_df[,i])                        # expected
    abline(v=male_unknown_originales[i], col = "red") # observed
  }
)
...
# Females 
y_females <- y[which(sex==2),] # Subset females (sex=2)
#dim(y_females)

dev.off()

# Counting Breeders  Event number = 2

# Expected female breeders from model
#Remember, in the model we used = y + 1; event 2 becomes a 3 (lines 51-61)
female_breeders <- colSums(females[,,]==3, dims = 1) 
female_breedersT <- t(female_breeders)
female_breedersT_df <- as.data.frame(female_breedersT) 
# Observed female breeders in the original dataset 
female_breeders_originales <- as.numeric(colSums(y_females[,]==2,dims=1))

## Histogram
colnames(female_breedersT_df) <- (1:11)
jpeg("female_breeders.jpg", width = 12, height = 8, units = "in", res = 300)
female_breeders_hist <- recordPlot(
  par(mfrow=c(2,6)),
  for(i in 1:11){
    hist(female_breedersT_df[,i])                        # expected
    abline(v=female_breeders_originales[i], col = "red") # observed
  }
)

dev.off()

# Counting Breeders.  Event number = 3

# Expected female floaters in the model
# Remember, in the model we used = y + 1; event 3 becomes a 4 (lines 51-61)
female_floaters<- colSums(females[,,]==4, dims = 1) 
female_floatersT <- t(female_floaters)
female_floatersT_df <- as.data.frame(female_floatersT) 
# Observed female floaters in the original dataset 
female_floaters_originales <- as.numeric(colSums(y_females[,]==3,dims=1))

## Histogram
colnames(female_floatersT_df) <- (1:11)
jpeg("female_floaters.jpg", width = 12, height = 8, units = "in", res = 300)
female_floaters_hist <- recordPlot(
  par(mfrow=c(2,6)),
  for(i in 1:11){
    hist(female_floatersT_df[,i])                        # expected
    abline(v=female_floaters_originales[i], col = "red") # observed
  }
)

dev.off()

# Counting Unknown.  Event number = 4

# Expected females with unknown breeding status in the model
# Remember, in the model we used = y + 1; event 4 becomes a 5 (lines 51-61)
female_unknown <- colSums(females[,,]==5, dims = 1) # dimension 1 is considered as row.
female_unknownT <- t(female_unknown)
female_unknownT_df <- as.data.frame(female_unknownT) ## format as dataframe.
# Observed females with unknown breeding status in the original dataset 
female_unknown_originales <- as.numeric(colSums(y_females[,]==4,dims=1))


## Histogram
colnames(female_unknownT_df) <- (1:11)
jpeg("female_unknown.jpg", width = 12, height = 8, units = "in", res = 300)
female_unknown_hist <- recordPlot(
  par(mfrow=c(2,6)),
  for(i in 1:11){
    hist(female_unknownT_df[,i])                        # expected
    abline(v=female_unknown_originales[i], col = "red") # observed
  } 
)
